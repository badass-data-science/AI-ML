...
def test_find_model_version_locates_the_tagged_version(trained_pd_lead_model):
    client = MlflowClient(tracking_uri=trained_pd_lead_model["tracking_uri"])
...
def test_find_model_version_raises_for_an_unknown_pair(trained_pd_lead_model):
    client = MlflowClient(tracking_uri=trained_pd_lead_model["tracking_uri"])
    with pytest.raises(ValueError, match="No registered version"):
        find_model_version(client, "forex-lstm", "GBP/USD", "H1")


def test_load_keras_model_returns_a_working_model(trained_pd_lead_model):
    client = MlflowClient(tracking_uri=trained_pd_lead_model["tracking_uri"])
    resolved = find_model_version(client, "forex-lstm", "EUR/USD", "H1")
...
def test_load_test_predictions_returns_the_expected_arrays(trained_pd_lead_model, tmp_path):
    client = MlflowClient(tracking_uri=trained_pd_lead_model["tracking_uri"])
    resolved = find_model_version(client, "forex-lstm", "EUR/USD", "H1")
